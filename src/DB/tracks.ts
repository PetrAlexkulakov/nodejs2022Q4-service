...
export const createOneTrack = (name: string, artistId: string | null, albumId: string | null, duration: number) => {
  return {
    id: generateUniqueId(),
    name,
...
const createTracks = (): Track[] => {
  ...
  for (let i = 0; i < 10; i++) {
    tracks.push(createOneTrack(`name${i + 1}`, null, null, Math.floor(Math.random() * 10)));
  }
